...
public class lg_touchpad extends View {
    ...
    public class RepeatHandler extends Handler {
        ...
        RepeatHandler(lg_touchpad trackpadView, RepeatHandler repeatHandler) {
            this();
        }
        ...
        public void handleMessage(Message message) {
            switch (message.what) {
                case 0:
                    lg_touchpad.this.sendEvent(lg_touchpad.this.mSwipeDirection, false);
                    removeMessages(0);
                    if (lg_touchpad.this.mCurrentRepeatPeriod > 0) {
                        sendEmptyMessageDelayed(0, (long) lg_touchpad.this.mCurrentRepeatPeriod);
                        return;
                    }
                    return;
                default:
    ...
    public lg_touchpad(Context context, AttributeSet attributeSet) {
        ...
        this.mRepeatHandler = new RepeatHandler(lg_touchpad.this, null);
    }
        ...
        if (motionEvent.getPointerId(motionEvent.getActionIndex()) == this.mGamepadPointerId) {
            if (this.mSwipeDirection == 0) {
                if (!this.mLongPressingCenter) {
                    this.mListener.doKey(KeyCode.ENTER);
                } else {
    ...
    private void sendEvent(int i, boolean z) {
        switch (i) {
            case 0:
                ...
                return;
            case 1:
                this.mListener.doKeyLeft();
                return;
            case 2:
                this.mListener.doKeyRight();
                return;
            case 3:
                this.mListener.doKeyUp();
                return;
            case 4:
                this.mListener.doKeyDown();
                return;
            default:
                return;
        }
